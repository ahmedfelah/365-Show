...
public class ChiefsPlayer {
    ...
    public lazy var acvFullscreen   :ACVFullscreen   = .activatedLock {
        didSet {
              ...
              startWithSubtitleAt subtitleIndex:Int? = nil)
    {
		...
		if self.selectedSource.timelines != nil {
			...
			timeObserverToken = self.player.addPeriodicTimeObserver(forInterval: CMTimeMakeWithSeconds(1, preferredTimescale: 1000), queue: .main) { [weak self] time in
