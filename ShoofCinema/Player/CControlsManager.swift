...
extension CControlsManager {
    ...
    func forwardBtnAction () {
        if let action = ChiefsPlayer.shared.delegate?.chiefsplayerForwardAction(true, timeline: ChiefsPlayer.shared.currentTimeline) {
            performAction(action: action)
        }
    }
    func backwardBtnAction () {
        if let action = ChiefsPlayer.shared.delegate?.chiefsplayerBackwardAction(true, timeline: ChiefsPlayer.shared.currentTimeline) {
            performAction(action: action)
        }
    }
    
    func episodesBtnAction () {
        ChiefsPlayer.shared.delegate?.chiefsplayerShowEpisodes()
    }
    ...
    func seek(by seconds:Int) {
        ...
        if let current = player?.currentTime() {
            player?.seek(to: current + CMTime.init(seconds: Double(seconds), preferredTimescale: 1000))
        }
    }
    ...
    func seek(to interval:Int) {
        ...
        player?.seek(to: CMTime.init(seconds: Double(interval), preferredTimescale: 1000))
    }
...
extension CControlsManager {
    ...
    public func controlsEpisodesHideButtonToggle(to isHidden: Bool) {
        delegates.forEach({$0?.controlsEpisodesHideButtonToggle(to: isHidden)})
    }
}
...
extension CControlsManager {
    ...
    func update (controller observer:CControlsManagerDelegate) {
        ...
        let currentTimeline = chiefsPlayer.currentTimeline
        ...
        if let backwardAction = chiefsPlayer.delegate?.chiefsplayerBackwardAction(false, timeline: currentTimeline) {
            observer.controlsBackwardActionDidChange(to: backwardAction)
        } else {
            observer.controlsBackwardActionDidChange(to: nil)
        }
        
        if let forwardAction = chiefsPlayer.delegate?.chiefsplayerForwardAction(false, timeline: currentTimeline) {
        ...
        if let subs = ChiefsPlayer.shared.selectedSource.subtitles , subs.count > 0 {
            ...
            for (thisIndex,sub) in subs.enumerated() {
                let action = UIAlertAction(
                    title: localized("subtitle_language.arabic"),
